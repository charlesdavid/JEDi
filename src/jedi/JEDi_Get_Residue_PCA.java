...
{
	final boolean doCORR, doPCORR, doReduce, verbose;
	boolean success, exist;
	String out_dir, file_name_head, path;
	final String directory, description, typeAlignment, Q = "COV", R = "CORR", P = "PCORR";
	double trace_COV, trace_CORR, trace_PCORR, trace_CORR_SPARSE, trace_PCORR_SPARSE, cond_COV, cond_prime, det_COV;
	final double st_threshold, FLOOR = Input_Parameters.FLOOR, NOISE_LEVEL = Input_Parameters.NOISE_LEVEL;
	int rank_COV, Residue_Index;
	...
	Matrix cov, corr, pcorr, inv_cov, rcov, rcorr, rpcorr, r_inv_cov, corr_sparse, pcorr_sparse, rcorr_sparse, rpcorr_sparse;
	 ...
	 */
	JEDi_Get_Residue_PCA(Matrix data, int num_modes, String type)
	{
	...
	public void Do_COV_PCA()
	{
		...
		if (typeAlignment.equals("Local"))
			{
				String out_dir_cov = out_dir + Q + File.separatorChar;
				create_Directory(out_dir_cov);

				String res_index = String.format("%03d", (Residue_Index));
				file_name_head = out_dir_cov + "Residue_" + res_index;

				path = file_name_head + "_top_" + number_of_modes + "_eigenvalues_COV.txt";
				if (verbose) List_IO.write_Double_List(eigenvalues_COV, path, 12);

				path = file_name_head + "_top_" + number_of_modes + "_eigenvectors_COV.txt.bz2";
				if (verbose) Matrix_IO.write_BZ2_Matrix(top_evectors_COV, path, 15, 12);

				write_top_evals_COV();

				path = file_name_head + "_covariance_matrix.txt.bz2";
				if (verbose) Matrix_IO.write_Matrix_adaptive_spacing(cov, path);
				...
				HeatMap hm_cov = new HeatMap(cov, file_name_head, "_Covariance_Matrix", "Variable 1", "Variable 2");
				hm_cov.createMatrixHeatmap();

				if (doReduce)
					{
						rcov = pca.get_Reduced_C_Matrix(cov);
						path = file_name_head + "_reduced_covariance_matrix.txt.bz2";
						if (verbose) Matrix_IO.write_Matrix_adaptive_spacing(rcov, path);

						HeatMap hm_rcov = new HeatMap(rcov, file_name_head, "Reduced_Covariance_Matrix", "Atom 1", "Atom 2");
	...
	{
		String out_dir_corr = out_dir + R + File.separatorChar;
		create_Directory(out_dir_corr);

		String res_index = String.format("%03d", (Residue_Index));
		file_name_head = out_dir_corr + "Residue_" + res_index;

		corr = pca.get_Correlation_Matrix_from_Covariance_Matrix(cov);
		path = file_name_head + "_correlation_matrix.txt.bz2";
		if (verbose) Matrix_IO.write_Matrix_adaptive_spacing(corr, path);

		HeatMap hm_corr = new HeatMap(corr, file_name_head, "_Correlation_Matrix", "Variable 1", "Variable 2");
		hm_corr.createMatrixHeatmap();

		if (doReduce)
			{
				rcorr = pca.get_Reduced_C_Matrix(corr);
				path = file_name_head + "_reduced_correlation_matrix.txt.bz2";
				if (verbose) Matrix_IO.write_Matrix_adaptive_spacing(rcorr, path);

				HeatMap hm_rcorr = new HeatMap(rcorr, file_name_head, "_Reduced_Correlation_Matrix", "Atom 1", "Atom 2");
	...
	{
		String out_dir_corr = out_dir + R + File.separatorChar + "sparse" + File.separatorChar;
		create_Directory(out_dir_corr);
		...
		HeatMap hm_scorr = new HeatMap(corr_sparse, out_dir, "Sparsified_Correlation_Matrix", "Variable 1", "Variable 2");
		...
		if (doReduce)
			{
				...
				HeatMap hm_rcorr = new HeatMap(rcorr_sparse, out_dir, "Reduced_Sparsified_Correlation_Matrix", "Atom 1", "Atom 2");
				hm_rcorr.createMatrixHeatmap();
			}
	...
	{
		String out_dir_pcorr = out_dir + P + File.separatorChar;
		create_Directory(out_dir_pcorr);

		String res_index = String.format("%03d", (Residue_Index));
		file_name_head = out_dir_pcorr + "Residue_" + res_index;

		pcorr = pca.get_Partial_Correlation_Matrix(inv_cov);
		path = file_name_head + "_partial_correlation_matrix.txt.bz2";
		if (verbose) Matrix_IO.write_Matrix_adaptive_spacing(pcorr, path);

		HeatMap hm_pcorr = new HeatMap(pcorr, file_name_head, "_Partial_Correlation_Matrix", "Variable 1", "Variable 2");
		hm_pcorr.createMatrixHeatmap();

		if (doReduce)
			{
				rpcorr = pca.get_Reduced_C_Matrix(pcorr);
				path = file_name_head + "_reduced_partial_correlation_matrix.txt.bz2";
				if (verbose) Matrix_IO.write_Matrix_adaptive_spacing(rpcorr, path);

				HeatMap hm_rpcorr = new HeatMap(rpcorr, file_name_head, "_Reduced_Partial_Correlation_Matrix", "Atom 1", "Atom 2");
	...
	{
		String out_dir_pcorr = out_dir + P + File.separatorChar + "sparse" + File.separatorChar;
		create_Directory(out_dir_pcorr);
		...
		HeatMap hm_pcorrS = new HeatMap(pcorr_sparse, out_dir, "Sparsified_Partial_Correlation_Matrix", "Variable 1", "Variable 2");
		...
		if (doReduce)
			{
				...
				HeatMap hm_rpcorrS = new HeatMap(rpcorr_sparse, out_dir, "Reduced_Sparsified_Partial_Correlation_Matrix", "Atom 1", "Atom 2");
				...
				HeatMap hm = new HeatMap(rdiff, out_dir, "Difference of Reduced Correlation and  Partial Correlation Matrices", "Variable 1", "Variable 2");
				...
				hm = new HeatMap(radjacency, out_dir, "Adjacency Matrix derived from difference of Reduced Correlation and  Partial Correlation Matrices", "Atom 1", "Atom 2");
		...
		HeatMap hm = new HeatMap(diff, out_dir, "Difference of Correlation and  Partial Correlation Matrices", "Variable 1", "Variable 2");
		hm.createMatrixHeatmap();
		...
		hm = new HeatMap(adjacency, out_dir, "Adjacency Matrix derived from difference of Correlation and  Partial Correlation Matrices", "Variable 1", "Variable 2");
		hm.createAdjacencyHeatmap();
	...
	private void get_eigenvalues_COV()
	{
		...
		first = eigenvalues_COV.get(0);
		last = eigenvalues_COV.get(size - 1);
		cond_COV = (first / last);
	}
	...
	private void write_top_evals_COV()
	{
		...
		SCREE_Plot.createChart(file_name_head + "_", "Scree_Plot_COV", screePlot);
	}
	...
	private void construct_PCA_Modes_COV()
	{
		...
		COLLECTIVITY_Plot.createPlot(file_name_head + "_", "Eigenvector_Collectivity_COV", collectivity_cov);
	...
	private void write_top_evals_CORR()
	{
		...
		SCREE_Plot.createChart(file_name_head + "_", "Scree_Plot_CORR", screePlot);
	}
	...
	private void get_top_evects_and_reverse_CORR()
	{
		...
		if (verbose) Matrix_IO.write_BZ2_Matrix(top_evectors_CORR, path, 15, 12);
	}
	...
	private void construct_PCA_Modes_CORR()
	{
		...
		COLLECTIVITY_Plot.createPlot(file_name_head + "_", "Eigenvector_Collectivity_CORR", collectivity_corr);
	...
	private void write_top_evals_PCORR()
	{
		...
		SCREE_Plot.createChart(file_name_head + "_", "Scree_Plot_PCORR", screePlot);
	}
	...
	private void get_top_evects_and_reverse_PCORR()
	{
		...
		if (verbose) Matrix_IO.write_BZ2_Matrix(top_evectors_PCORR, path, 15, 12);
	}
	...
	private void construct_PCA_Modes_PCORR()
	{
		...
		COLLECTIVITY_Plot.createPlot(file_name_head + "_", "Eigenvector_Collectivity_PCORR", collectivity_pcorr);
	 ...
	 * @return
	 */
	public List<Double> get_Eigenvalues_COV()
	{
	 ...
	 * @return
	 */
	public List<Double> get_Eigenvalues_CORR()
	{
	 ...
	 * @return
	 */
	public List<Double> get_Eigenvalues_PCORR()
	{
...
//---------------------------------------------------------------------------------------------------------
	 ...
	 * @return
	 */
	public Matrix get_Top_evectors_COV()
	{
	 ...
	 * @return
	 */
	public Matrix get_Top_evectors_CORR()
	{
		return top_evectors_CORR;
	}

	/**
	 * Returns the Top eigenvectors from the SPARSE CORR analysis
	 *
	 * @return
	 */
	public Matrix get_Top_evectors_CORR_SPARSE()
	{
	 ...
	 * @return
	 */
	public Matrix get_Top_evectors_PCORR()
	{
		return top_evectors_PCORR;
	}

	/**
	 * Returns the Top eigenvectors from the SPARSE PCORR analysis
	 *
	 * @return
	 */
	public Matrix get_Top_evectors_PCORR_SPARSE()
	{

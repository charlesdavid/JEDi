...
public class JEDi_Get_Distances_for_Atom_Pairs
	{
		...
		int ROWS, ROWS_dp, COLS, number_of_atoms, number_of_atom_pairs;
		...
		public JEDi_Get_Distances_for_Atom_Pairs(String dir, String desc, Matrix data, List<Integer> atm_list1, List<Integer> atm_list2)
			{
				...
				out_dir = directory + "JEDi_RESULTS_" + description + File.separatorChar + "dpPCA" + File.separatorChar;
		...
		public Matrix Get_Ref_Distances()
			{
				...
				String path = out_dir + "ss_" + number_of_atom_pairs + "_Reference_Atom_Pair_Distances.txt";
				Matrix_IO.write_Matrix(ref_distances.transpose(), path, 12, 6);
		...
		public Matrix get_Distances()
			{
						...
						for (int i = 0; i < number_of_atom_pairs; i++)
							{
								...
								double x = X_vectors.get(atom1, observation);
								double y = X_vectors.get(atom1 + number_of_atoms, observation);
								double z = X_vectors.get(atom1 + 2 * number_of_atoms, observation);

								double xr = X_vectors.get(atom2, observation);
								double yr = X_vectors.get(atom2 + number_of_atoms, observation);
								double zr = X_vectors.get(atom2 + 2 * number_of_atoms, observation);
				...
				String path = out_dir + "ss_" + number_of_atom_pairs + "_Atom_Pair_Distances.txt";
				Matrix_IO.write_Matrix(distances.transpose(), path, 12, 6);
				return distances;
			}
	}

...
/**
 * So you've got some 2d points and you want to convolve them with a gaussian kernel to generate a pdf.
 * What bandwidth of kernel do you use? Well, there's some criteria, like MSIE or some such, that you should usually try to minimize.
 * That is what this class does. Give it some points and it will give you the optimal kernel bandwidth.
...
public class DiagonalBandwidthSelector2d
{
	...
	static double kernel_resolution = 10d;

	public DiagonalBandwidthSelector2d()
	{
	}

	public static double[] computeBandwidth(double[] points, int off, int numPoints, double[] bounds)
	{
		return computeBandwidth(points, off, numPoints, bounds, 1 << 8);
	}

	public static double[] computeBandwidth2(double[] points, int off, int numPoints, double[] bounds, int quant) throws MathException
	{
		...
		if (Double.isNaN(tStar))
		{
			...
			} // catch (MathException ex)
			{
				...
				bandwidths[0] = ((bounds[2] - bounds[0]) / 5);
				bandwidths[3] = ((bounds[3] - bounds[1]) / 5);
				// System.err.println("WARNING: Kernel Bandwidths could not be optimized: Possibly not enough points were used.");
				// System.err.println("Kernel Bandwidths have been set to: " + bandwidths[0] + ", " + bandwidths[3]);
				return bandwidths;
			}
		}
	...
	public static double[] computeBandwidth(double[] points, int off, int numPoints, double[] bounds, int quant)
	{
		if (bounds == null) bounds = computeBounds(points, off, numPoints, 0.25);
		bandwidths = new double[] { 0, 0, 0, 0 };
		bandwidths[0] = ((bounds[2] - bounds[0]) / kernel_resolution);
		bandwidths[3] = ((bounds[3] - bounds[1]) / kernel_resolution);
		return bandwidths;
	}

	public static void set_Resolution(double bandwidth)
	{
		kernel_resolution = bandwidth;
	}
